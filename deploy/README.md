...
1. **Checkout.** Copy or clone the release into its own directory (the dev tree keeps its own `.env` and ports):
   `cp -r RazorpayAIBuildathon AegisProd-RazorpayAIBuildathon && cd AegisProd-RazorpayAIBuildathon`
...
Redeploy: refresh the checkout, repeat step 3, then `pm2 restart ecosystem.config.js`. Rollback: `pm2 stop all`
