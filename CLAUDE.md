...
5. Verdict: **GREEN** (tag `task-NN-done` if missing, unlock the next task, state its number and any prerequisites) or **RED** (list concrete fixes with file:line; the same task is re-handed to Codex).
