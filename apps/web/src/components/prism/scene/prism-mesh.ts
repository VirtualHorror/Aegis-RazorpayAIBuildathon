/**
 * Procedural, subtly filleted prism geometry, ported from `vercel-labs/vgpu`
 * (`apps/docs/.../prism-background/scene/prism-mesh.ts`).
 *
 * Intent: `constants.ts` owns the ideal triangular solid the CPU ray tracer and the glass shaders
 * measure the optical path against. This file only rounds the *visible* mesh inward, so the bevel can
 * catch the environment without ever moving a light-producing face outside its analytically traced
 * plane.
 *
 * Flow: replace the triangular cross-section with tangent circular arcs, then blend that outline into
 * inset front and back caps with quarter-round rings. Four segments resolve each rounded direction;
 * the long straight runs are split independently so neither the bevel strips nor the cap triangles
 * span a whole side of the prism.
 */

import type { Geometry, Gpu } from "vgpu";
import { geometry } from "vgpu";

import { PRISM_BACK_Z, PRISM_FRONT_Z, PRISM_TRIANGLE, type Triangle, type Vec2, type Vec3 } from "./constants";
...
/**
 * Vertices and indices for the rounded prism, wound counter-clockwise from outside so `cull: "back"`
 * keeps the faces that face the camera.
 */
export function prismMeshData(
  ...
  // Stop a fraction short of pi/2 so the tiny cap corner arcs retain positive area instead of
  // collapsing every sample at a corner onto the same point.
  ...
  for (let step = PRISM_BEVEL_SEGMENTS; step >= 0; step--) {
    ...
    addRing(theta, backZ + radius - (radius * Math.sin(theta)) / maxSine, -Math.sin(theta));
  }
  for (let step = 0; step <= PRISM_BEVEL_SEGMENTS; step++) {
    const theta = (maxTheta * step) / PRISM_BEVEL_SEGMENTS;
    addRing(theta, frontZ - radius + (radius * Math.sin(theta)) / maxSine, Math.sin(theta));
  }
...
/**
 * The five ideal outward planes whose intersection is the optical solid, as `(nx, ny, nz, d)` with
 * `dot(n, point) <= d` inside. Both glass shaders trace against these, not against the rounded mesh.
 */
...
export function prismGeometry(gpu: Gpu, label: string): Geometry {
  ...
  return geometry(gpu, {
    label,
    buffers: [
      { data: vertices, stride: PRISM_VERTEX_STRIDE, attributes: { position: "float32x3", normal: "float32x3" } },
    ],
    indices,
  });
}
...
function roundedTriangleContour(triangle: Triangle, radius: number): ContourPoint[] {
  ...
  const arcs = corners.map((corner, index) => {
    ...
    const bisector = normalize2([towardPrevious[0] + towardNext[0], towardPrevious[1] + towardNext[1]]);
    const center: Vec2 = [corner[0] + bisector[0] * centerDistance, corner[1] + bisector[1] * centerDistance];
    const start: Vec2 = [
      corner[0] + towardPrevious[0] * tangentDistance,
      corner[1] + towardPrevious[1] * tangentDistance,
    ];
    const end: Vec2 = [corner[0] + towardNext[0] * tangentDistance, corner[1] + towardNext[1] * tangentDistance];
    ...
    return Array.from({ length: PRISM_CORNER_SEGMENTS + 1 }, (_, step): ContourPoint => {
      ...
      return { position: [center[0] + normal[0] * radius, center[1] + normal[1] * radius], normal };
    });
  });

  return arcs.flatMap((arc, index) => {
    const end = arc[arc.length - 1]!;
    const nextStart = arcs[(index + 1) % arcs.length]![0]!;
    const edgeNormal = outwardNormal(corners[index]!, corners[(index + 1) % corners.length]!);
