...
export function KillSwitchPill({ enabled }: { enabled: boolean | null }) {
  if (enabled === null) {
    return (
      <span className="inline-flex h-7 shrink-0 items-center whitespace-nowrap gap-1 rounded-full border border-border px-2 text-[11px] text-fg-muted" title="Kill switch state unknown">
  ...
  return (
    <Link
      href="/settings"
      className={`inline-flex h-7 shrink-0 items-center whitespace-nowrap gap-1 rounded-full border px-2 text-[11px] font-medium ${enabled ? "border-danger bg-danger/10 text-danger" : "border-border text-fg-muted hover:text-fg"}`}
