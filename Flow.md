...
## F11. Simulator and demo [planned T5, T23]

`pnpm sim <scenario> [--dupes N --burst N --chaos llm_down]` → `scripts/simulate.ts` builds payloads from `scenarios/*.ts` (payment.failed 3DS intl, cart drop-off, subscription.halted, invoice.expired ₹4.2L, dispute.created, payment.captured for attribution) → signs → POSTs → prints a summary table (accepted / duplicate / rejected, latency p50/p95). `pnpm demo` runs the storyboard used in the video.
