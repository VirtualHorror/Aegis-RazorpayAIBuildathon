...
| the prism hero is flat | no WebGPU adapter on this machine | expected; the still under the canvas is the same scene, projected from the same constants and the same ray tracer (D-087) |
