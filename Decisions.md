...
### D-077 · Tenant webhook secrets live in `guardrail_config`, hidden by row-level security (2026-09-06)

**Context.** Sandbox / BYOK mode (T25) needs one server-side secret per merchant account: the Razorpay webhook secret the ingress verifies deliveries with. The brief asks for it in `guardrail_config` under `sandbox_secret_<account_id>`. That table is deliberately readable by `aegis_readonly` (0002) so Ask Aegis can answer questions about limits, and `GET /api/v1/guardrails` renders every row on the settings page — both of which would print the secret.
**Choice.** Keep the row where the brief puts it, and make the two readers blind to it: the guardrails route filters `key NOT LIKE 'sandbox\_secret\_%'`, and `0005_sandbox_secret_isolation.sql` enables RLS on the table with one SELECT policy that hides the same prefix. The table owner (`aegis`) bypasses RLS because the table is not `FORCE`d, so the ingress lookup and the route see everything. `loadGuardrailConfig` ignores unknown keys, so the module bounds are unaffected.
**Consequences.** No new table, no new grant, one reversible migration. Any future non-owner role sees the bounds and nothing else. A test connects as `aegis_readonly` to `aegis_test` and proves the policy; it failed before the migration existed.

### D-078 · A caller's model key goes to the provider that issued it, on its own client (2026-09-06)

**Context.** `x-aegis-llm-key` lets a merchant pay for their own model calls. The boot client is built once from `.env`, may point `OPENAI_BASE_URL` at a proxy the deployer chose, and holds the circuit breaker every request shares.
**Choice.** `createLlmClient(config, logger, { apiKey })` builds a *separate* resilient client for the caller: `resolveByokProvider` honours an explicit `AEGIS_LLM_PROVIDER` (so `stub` cannot be undone from a browser) and otherwise reads the key's own prefix (`sk-ant-` → Anthropic, else OpenAI); `byokAdapterOptions` never sets a base URL for OpenAI, so the caller's credential reaches api.openai.com and never the deployer's proxy. `ByokLlmRegistry` caches one client per key fingerprint (bounded, LRU) so retries and the breaker work per tenant, and `remember`/`byFingerprint` let a queued compliance scan run on the key without the key ever touching PostgreSQL (DV-013). The header is redacted in logs and validated to printable ASCII of 20–512 characters before it can reach an HTTP client.
**Consequences.** A wrong key degrades only that caller's requests (proven live: `openai_AuthenticationError: 401` with the rules fallback), never the deployment's. The model id still comes from `.env` (`OPENAI_MODEL`); a deployment pinned to a proxy-only model id must set a public one for BYOK to succeed — documented in the README. Keys survive only in process memory; a restart silently returns queued work to the boot client.

### D-079 · Browser-side keys are stored in localStorage and persisted only after the API accepts the secret (2026-09-06)

**Context.** The modal holds three values with three different homes: the account id and webhook secret belong on the API, the model key belongs on every request. The brief asks for all three in `localStorage`.
**Choice.** `lib/sandbox.ts` is a plain store (`aegis.sandbox`, parsed defensively, Demo mode on anything unreadable) with no React import so `lib/api.ts` can use it from server pages; the hook is a separate `"use client"` module (B-024). `Save & go live` validates with the same rules as the route, POSTs the secret, and writes localStorage only on a 200 — a failed save must not leave the browser claiming Live mode against a secret the ingress does not have. Switching to Demo keeps the typed values so the toggle is cheap; `Forget keys` clears them. The secret's floor is 8 characters (DV-014). No new dependency: the modal portals through `react-dom`.
**Consequences.** Keys live in one browser and can be cleared by the user; nothing here can leak to another origin. The stored secret on the API outlives the browser's copy by design and is rotated by saving again.

### D-080 · Task 26 (control-plane authentication) is declined for the MVP; the boundary is documented, not solved (2026-09-06)

**Context.** The T25 security review raised F-028: every `/api/v1/*` write — guardrail edits, the kill switch, approvals, compliance decisions, `POST /api/v1/sandbox/keys` — trusts whoever reaches the API and records the body's `actor`. With the instance going public at `aegis.nabhanyubm.tech` the project owner asked whether global authentication should become Task 26 before the deadline.
**Choice.** Decline it. State the boundary in `Architecture.md §11` and in the README next to the Live-mode instructions, label the hosted instance an evaluator sandbox, and keep F-028 as the first post-MVP task. Authentication is cross-cutting — principals and sessions, `actor` derived from the principal on every write route, tenant ownership of `account_id`, the dashboard's login and key handling — and would be the one task built without the verification pass every other task got. A half-verified gate invites trust the code cannot honour; a stated absence does not. What the sandbox relies on instead is already built and tested: TLS, a rate limit keyed by the real client (D-081), row-level security and fingerprints for stored secrets (D-077), model keys that never touch the database (D-078), and simulation-only outbound (C-B7) so nothing here can message a customer or move money.
**Consequences.** No tenant isolation: anyone who reaches the API can edit a guardrail, decide an approval or rotate another account's stored secret. The README tells Live-mode users to use a test-mode Razorpay account and a revocable model key. The scoreboard, ledger and audit trail of the hosted instance are shared and resettable, and are described as such.

### D-081 · The hosted instance is a sibling production checkout under PM2 behind nginx with Let's Encrypt (2026-09-06)

**Context.** Evaluators need URLs. The buildathon VM is the only host, PostgreSQL is already on it, `sudo` is a human step (the agent cannot install packages), and the dev tree must keep working for `pnpm dev` and the drills.
**Choice.** `/home/nabhanyu/Downloads/AegisProd-RazorpayAIBuildathon` is a copy of the dev tree at `v1.1.0` with its own `.env`. `ecosystem.config.js` (repo root) runs the API as `node --import tsx src/server.ts` and the dashboard as `next start`, both bound to 127.0.0.1, under PM2 fork mode with one instance each (the worker leases, the breaker and the BYOK registry are in-process state). nginx (`deploy/nginx/aegis.conf`) terminates TLS: `aegis.nabhanyubm.tech` → :3000, `api.aegis.nabhanyubm.tech` → :4000, with `Host`, `X-Real-IP`, `X-Forwarded-For` and `X-Forwarded-Proto` set and buffering off for the SSE stream; `certbot --nginx` adds the certificate and the redirect. The API gets a `TRUST_PROXY` knob (`loopback` on the host, off by default) so `request.ip` — and the rate limiter — see the client instead of 127.0.0.1, without letting a direct caller in development pick its own bucket via `X-Forwarded-For`. `deploy/setup-server.sh` is the single sudo step (apt, site, certbot, `pm2 startup`). Three things are deliberate: `NODE_ENV` stays `development`, because C-D5 keeps the simulator routes out of production mode and the dashboard's Run demo needs them — the environment pill says so honestly; Next.js reads env files only from `apps/web`, so `deploy/sync-web-env.sh` copies the `NEXT_PUBLIC_*` lines of the root `.env` into `apps/web/.env.production` before every build and the root `.env` stays the one place to edit; the prod `.env` gets its own `RAZORPAY_WEBHOOK_SECRET` and `X402_SIM_SECRET`, so the public ingress does not accept the placeholder from `.env.example`.
**Consequences.** Deploy is copy → edit `.env` → sync → build → `pm2 restart`; rollback is `pm2 stop all` or the previous tag. Both trees share the one database, so only one API may run at a time — port 4000 is the lock. Server components fetch the API through the public hostname like the browser does, so pages render their "API offline" states until TLS is in place.

### D-082 · The hero borrows the vgpu reference's rendering techniques, not its scene (2026-09-06)
...
### D-088 · The production tree owns its own `.env`, logs and backups; the deploy mirror excludes them (2026-09-07)

**Context.** D-081 built the hosted instance as a sibling checkout with its own `.env` — its own hostnames, `API_HOST=127.0.0.1`, `TRUST_PROXY=loopback`, and its own `RAZORPAY_WEBHOOK_SECRET` and `X402_SIM_SECRET` so the public ingress does not accept the placeholder from `.env.example`. Nothing enforced that. Refreshing the checkout was written as "refresh the checkout", and the mirror that grew into that gap — `rsync -av --delete --exclude node_modules --exclude .git --exclude .next` — copies `.env` like any other file. It ran, and the production tree silently became the dev tree: `NEXT_PUBLIC_API_URL=http://localhost:4000` was inlined into the client bundle by the next build, `WEB_ORIGIN=http://localhost:3000` narrowed CORS to an origin no browser would ever send, `API_HOST=0.0.0.0` published the API outside nginx, `TRUST_PROXY` disappeared so the rate limiter bucketed every caller as 127.0.0.1, and both secrets reverted to the values committed in `.env.example` (B-029).
**Choice.** `deploy/sync-prod.sh` is the mirror, and the exclude list is the decision. `.env`, `.env.bak-*` and `apps/web/.env.production` stay prod-owned because they *are* the environment; `logs/` stays because PM2 writes there and `ecosystem.config.js` names those paths, so deleting the directory can take the restart with it; `backups/` stays because it holds the production database dumps; `node_modules`, `.git` and `.next` are per-tree by definition and a copied `.next` is what caused the earlier crash loop. `--delete` stays — it is what keeps files deleted in git from lingering in production — so the script refuses to run unless the destination has an `ecosystem.config.js` and an `.env`, and refuses to sync a tree onto itself. It finishes by running `deploy/sync-web-env.sh` *in the destination*, so `apps/web/.env.production` is rewritten from the production `.env` rather than left to the next person to remember.
**Consequences.** Redeploy is one command plus a build. The two `.env` files are only ever equal by coincidence now, which means prod's secrets have to be set once, by hand, at checkout time — `.env.example`'s placeholders are public, so an instance still holding them is accepting forged webhooks from anyone who can read the repository. `deploy/README.md` step 1 says so, and `bash deploy/sync-prod.sh --dry-run` prints what `--delete` would remove before anything is written.
