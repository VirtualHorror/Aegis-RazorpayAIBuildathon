...
| F-005 | Idempotent webhook ingress (HMAC, dedupe, outbox) | T3 | Codex | implemented | `pnpm --filter @aegis/api test -- ingress` → 6 files passed, 27 tests passed (including 20 concurrent identical POSTs → 1 row, duplicate_count=19, 1 job); runtime alternate-port curl → `401 {"error":"invalid_signature"}`; full repository gates recorded in `HANDOFF3.md` |
